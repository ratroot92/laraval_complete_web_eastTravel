  ...
  <tbody>
    ...
    <tr>
      ...
      <td> @foreach($item->GetActivityCategory as $cat)
        {{$cat->name}}<br>
      @endforeach</td>
      <td><img src="{{str_replace('http://localhost:8000/public/','http://localhost:8000/',$item->banner)}}" alt="{{$item->event_name}}" class="img" style="width: 100%;height: 80px"></td>
      {{--   <td>{{$item->country}}</td>  --}}
      ...
      <td>{{$item->price}}</td>
      {{--  <td>{{$item->date}}</td> --}}
      <td><a href="{{ url('view_update_event/updateEvent')}}/{{$item->id}}" class="sb2-2-1-edit"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a>
