         ...
         <div class="row ">
            <div class="col-md-12 ">
               <div class="radio-toolbar border-dark d-fle">
                  ...
                  <a class="custom_anchors font-weight-bold" href="{{route('all_activities')}}" id="activities" >Activities</a>
...
//global_data
var type_name="activities";
var activities_count="{{ count($activities) }}"
//end glocal data

            var url_array =window.location.pathname.split("/");
             var key2=url_array[5];
            var key1=url_array[4];
            if(url_array[3]=='all_activities'){


               console.log(activities_count)
               $('#message_div').show();
    $('#message_div').empty().append('<i class="fa fa-list-alt text-danger  fa-2x mr-5" aria-hidden="true"></i> found all '+type_name+' ('+activities_count+') <span class="font-weight-bold text-danger ml-2"></span>');
   ...
   if(url_array[4]=='city'){
    ...
    $('#message_div').show();
    if(activities_count == '0' || activities_count == 0){
...
$('#message_div').empty().append('<i class="fa fa-list-alt text-danger  fa-2x mr-5" aria-hidden="true"></i> no   '+type_name+' results for price range  <span class="font-weight-bold text-danger ml-2">[ min price = '+minprice+' &&  maxprice = '+maxprice+']</span>');
   ...
   $('#message_div').empty().append('<i class="fa fa-list-alt text-danger  fa-2x mr-5" aria-hidden="true"></i> found all '+type_name+' ('+activities_count+') results for price range  <span class="font-weight-bold text-danger ml-2">[ min price = '+minprice+' &&  maxprice = '+maxprice+']</span>');
  }
...
<script>
   $(document).ready(function (){
      $('#all_cities').on('click',function(){
        window.location.href = "{{URL::to('booknow/all_activities')}}";
      });
