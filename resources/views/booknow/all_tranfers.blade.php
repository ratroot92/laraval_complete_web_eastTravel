                     ...
                     <div class="hot-page2-alp-l-com1 hot-page2-alp-p4">
                        <form>
                           <div id="cities">
                              <ul class="" >
                                 <li>
                                    <a href="{{route('all_transfers')}}" style="padding:0px!important;margin:0px!important;color:white; border:none;">
          ...
          <div class="row ">
            <div class="col-md-12 ">
               <div class="radio-toolbar border-dark d-fle">
                  ...
                  <input type="radio" id="radioActivites" name="options" value="3" checked>
                  ...
                  <a class="custom_anchors font-weight-bold" href="{{route('all_transfers')}}" id="transfers"  >Transfers</a>
               </div>
            </div>
         </div>
         <div class="row">
...
//global_data
var type_name="transfers";
var transfers_count="{{ count($transfers) }}"
//end glocal data

            var url_array =window.location.pathname.split("/");
             var key2=url_array[5];
            var key1=url_array[4];
            if(url_array[3]=='all_transfers'){


               console.log(transfers_count)
               $('#message_div').show();
    $('#message_div').empty().append('<i class="fa fa-list-alt text-danger  fa-2x mr-5" aria-hidden="true"></i> found all '+type_name+' ('+transfers_count+') <span class="font-weight-bold text-danger ml-2"></span>');
   ...
   if(url_array[4]=='city'){
    ...
    $('#message_div').show();
    if(transfers_count == '0' || transfers_count == 0){
...
$('#message_div').empty().append('<i class="fa fa-list-alt text-danger  fa-2x mr-5" aria-hidden="true"></i> no   '+type_name+' results for price range  <span class="font-weight-bold text-danger ml-2">[ min price = '+minprice+' &&  maxprice = '+maxprice+']</span>');
   ...
   $('#message_div').empty().append('<i class="fa fa-list-alt text-danger  fa-2x mr-5" aria-hidden="true"></i> found all '+type_name+' ('+transfers_count+') results for price range  <span class="font-weight-bold text-danger ml-2">[ min price = '+minprice+' &&  maxprice = '+maxprice+']</span>');
  }
...
<script>
   $(document).ready(function (){
      $('#all_cities').on('click',function(){
        window.location.href = "{{URL::to('booknow/all_transfers')}}";
      });
