            ...
            <table class="table">
                ...
                @foreach($cruises as $key=>$item)
                                      ...
                                      {{--   <td>{{$item->loc}}</td> --}}
                                        <td>{{$item->price}}</td>    
                                       {{--  <td>{{$item->date}}</td> --}}
<td><a href="/cruise/update/{{ $item->id }}" class="sb2-2-1-edit"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a>


<i onclick="confirm_delete('{{$item->id}}')" href="/cruise/delete/{{ $item->id }}" class="sb2-2-1-edit"><i class="fa fa-trash-o" aria-hidden="true"></i></i>

<a href="/cruise/detail/{{ $item->id }}" target="_blank" class="sb2-2-1-edit"><i class="fa fa-eye" aria-hidden="true"></i></a>
</td>
    ...
    <script>
        function confirm_delete(id) {
            ...
            if(c === true){
                window.location.href ="{{url('cruise/delete/')}}/"+id;
            }
        }
    </script>
